...
//-------------------------------------------------------------------------
//Subobtimal non segmental version
//each segment is decodified using the path carried on by the previous segments
//however the resulting path is not the optimal global path but the best solution for the segment
//segments must be sorted in ascending order to work properly
void viterbiDecodingNoSegmental(Config& config, hmm& actualHMM, SegCluster& cluster,
	SegServer& segTemp, StatServer& ss, FeatureServer & fs, LabelServer& labelServer, DoubleVector& actualViterbiProb)
{
	...
	while ((segment = cluster.getSeg()) != NULL){                               // Loop on each segment comming from the macro-class acoustic segmentation
		...
		if ((verbose) && (verboseLevel == 2)) cout << "Path probability: " << llp << endl;
		iprob++;
		copyPathInCluster(config, segTemp, path, pathoffset,actualHMM, labelServer, segment->begin(), segment->sourceName()); // Put the viterbi path (for the current macro segment)  in the cluster
		pathoffset = path.size();
	}//while segments	
}//Viterbi
//Optimal non segmental version
//each segment is decoding is obtained from the global path
void viterbiDecodingNoSegmentalOptimal(Config& config, hmm& actualHMM, SegCluster& cluster,
	SegServer& segTemp, StatServer& ss, FeatureServer & fs, LabelServer& labelServer, DoubleVector& actualViterbiProb)
{
	...
	//get the global decoding path
	iprob = 0;
	const ULongVector& path = va.getPath();
	unsigned long start , segLength;
	cluster.rewind();
	while ((segment = cluster.getSeg()) != NULL){                               // Loop on each segment comming from the macro-class acoustic segm
		start = pathoffset;
		segLength   = segment->length();
		copyPathInCluster(config, segTemp, path, start,segLength,actualHMM, labelServer, segment->begin(), segment->sourceName()); // Put the viterbi path (for the current macro segment)  in the cluster
		pathoffset = start + segLength;
...
void copyPathInCluster(Config& config, SegServer &segTemp, const ULongVector &path, unsigned long pathoffset, hmm &cHmm, LabelServer &labelServer, unsigned long start, String sourceName){

	unsigned long startV, stopV, indiceLoc;                                      // Begin/end of the current segment
	indiceLoc = path[pathoffset]; startV = start;                                                 // did we begin a segment ?
	for (unsigned long i = pathoffset + 1; i < path.size(); i++){
		if ((path[i] != indiceLoc)){                                 // End of a segment
			stopV = (i - pathoffset - 1) + start;
			Label l;
			l.setString(cHmm.getStateName(indiceLoc));
			unsigned long cl = labelServer.addLabel(l);
			SegCluster& clusterV = segTemp.getCluster(indiceLoc);
			...
			indiceLoc = path[i];
			startV = i + start - pathoffset;
		} //if		
	} //for
	// Deal with the last segment if needed
	stopV = (path.size() - pathoffset - 1) + start;
	Label l;
	l.setString(cHmm.getStateName(indiceLoc));
	unsigned long cl = labelServer.addLabel(l);
	SegCluster& clusterV = segTemp.getCluster(indiceLoc);
...
void copyPathInCluster(Config& config, SegServer &segTemp, const ULongVector &path, unsigned long pathoffset, unsigned long pathlength, hmm &cHmm, LabelServer &labelServer, unsigned long start, String sourceName){
	/*try {
*/
		unsigned long startV, stopV, indiceLoc, pathend;                                      // Begin/end of the current segment
		indiceLoc = path[pathoffset]; startV = start;                                                 // did we begin a segment ?
		pathend = pathoffset + pathlength;
		for (unsigned long i = pathoffset + 1; i < pathend; i++){
			if ((path[i] != indiceLoc)){                                 // End of a segment
				stopV = (i - pathoffset - 1) + start;
				Label l;
				l.setString(cHmm.getStateName(indiceLoc));
		...
		// Deal with the last segment if needed
		stopV = (pathend - pathoffset - 1) + start;
		Label l;
		l.setString(cHmm.getStateName(indiceLoc));
		unsigned long cl = labelServer.addLabel(l);
		SegCluster& clusterV = segTemp.getCluster(indiceLoc);
