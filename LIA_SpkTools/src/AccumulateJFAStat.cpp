...
//				Thread Routine
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
void *Statthread(void *threadarg) {
	struct Statthread_data *my_data;
	my_data = (struct Statthread_data *) threadarg;
	
	double *N = my_data->N;
	double *N_h = my_data->Nh;
	double *F_X = my_data->FX;
	double *F_X_h = my_data->FXh;
	...
	//Create a JFATranslate object for the complete XList
	JFATranslate ndxTable(fileList);
	...
	while((seg=selectedSegments.getSeg())!=NULL){
		...
		for (unsigned long idxFrame=0;idxFrame<seg->length();idxFrame++){
			...
			for(unsigned long k=0;k<n_distrib;k++) {
				...
				N_h[session*n_distrib+k]+=aPost[k];
				N[loc*n_distrib+k]   +=aPost[k];

				for (unsigned long i=0;i<vectSize;i++) {
					F_X_h[session*svSize+(k*vectSize+i)]+=aPost[k]*ff[i];
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
void JFAAcc::computeAndAccumulateJFAStatThreaded(unsigned long NUM_THREADS, Config &config){

	if (verboseLevel >= 1) cout << "(AccumulateJFAStat) Compute Statistics Threaded"<<endl;
	...
	struct Statthread_data *thread_data_array = new Statthread_data[NUM_THREADS];
	...
	for(unsigned long t=0; t<NUM_THREADS; t++){
		...
		thread_data_array[t].Nh=N_h;
		thread_data_array[t].FX=F_X;
		thread_data_array[t].FXh=F_X_h;
		...
		rc = pthread_create(&threads[t], &attr, Statthread, (void *)&thread_data_array[t]);
