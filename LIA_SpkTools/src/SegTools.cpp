...
void verifyClusterFile(SegServer& segmentsServer, FeatureServer& fs, Config& config)
{
	for (unsigned long clusterIdx = 0; clusterIdx < segmentsServer.getClusterCount(); clusterIdx++){  // For each cluster
		...
		while ((seg = cluster.getSeg()) != NULL)     
		{// For each segment
			unsigned long segStart = seg->begin();
			unsigned long endOfSource = fs.getFeatureCountOfASource(seg->sourceName());
			if ( segStart >= endOfSource )			
				throw Exception("Segment Start is out of feature file!!", __FILE__, __LINE__);
			
			if ((segStart + seg->length()) > endOfSource ){ // The end of the segment is after the last frame
			unsigned long newLength = endOfSource - segStart;
			if (debug || verbose)cout << "Warning File[" << seg->sourceName() << "], Truncate Segment begin:" << segStart << " length:" << seg->length() <<
				" file size:" << endOfSource << " new length:" << newLength << endl;
...
void   accumulateStatViterbi(FeatureServer &fs, ViterbiAccum &va, unsigned long beginIdx, unsigned long length, Config &config)
{
	...
	unsigned long endIdx = (beginIdx + length);
	for (long ifeature = beginIdx; ifeature <= (long)(beginIdx + length) - (long)viterbiBufferLength; ifeature += viterbiBufferLength)	
	//for (long ifeature = beginIdx; endIdx - ifeature >=  viterbiBufferLength; ifeature += viterbiBufferLength)
	...
	TransitionsFort, DoubleVector& TransitionsFaible, unsigned long NbState)
{
	...
	unsigned long endIdx = (begin + seg->length());
	for (long ifeature = begin; endIdx - ifeature >=  viterbiBufferLength; ifeature += viterbiBufferLength)
